...
def parse_comment_age_hours(date_label: str, now: datetime | None = None) -> float | None:
    ...
    m = re.match(r"^(\d+)\s*(h|hr|ч|год|г)$", raw)
    if m:
        return float(m.group(1))
    m = re.match(r"^(\d+)\s*(d|day|дн\.?|д\.?)$", raw)
    if m:
        return float(m.group(1)) * 24.0
    m = re.match(r"^(\d+)\s*(w|week|нед)$", raw)
    if m:
...
def _merge_rows(seen: dict[str, dict], rows: list[dict], order_counter: list[int]) -> int:
    ...
    for row in rows:
        ...
        if not text:
            continue
        pid = f"{author}:{hash(text) & 0xFFFFFFFF:x}"
        if pid in seen:
            continue
...
def _base_filter(comments, cfg, own_username: str = "") -> list[dict]:
    ...
    own_is_profile_id = len(own) <= 12 and own.isalnum()
    ...
    include_kw = _keyword_lines(
        cfg, "commentIncludeKeywords", "comment_include_keywords"
    )
    exclude_kw = _keyword_lines(
        cfg, "commentExcludeKeywords", "comment_exclude_keywords"
    )
    ...
    for c in comments:
        if skip_pinned and c.get("isPinned"):
            continue
        if skip_own and own and not own_is_profile_id and (c.get("author") or "").lower() == own:
            continue
        if not c.get("text"):
            continue
...
def comment_passes_filter(c: dict, cfg, own_username: str = "") -> bool:
    return bool(_base_filter([c], cfg, own_username))
...
def iterate_comments_top_down(
    ...
    def _emit_new():
        for c in sorted(storage.values(), key=lambda x: x.get("listOrder", 0)):
            ...
            processed.add(pid)
            st["scanned"] += 1
            if not comment_passes_filter(c, cfg, own_username):
                st["skippedFilter"] += 1
                continue
            st["yielded"] += 1
            yield c
